...
namespace realsense2_camera
{
    ...
    class Diagnostics
    {
        public:
            ...
            ~Diagnostics()
            {
                std::cout << "Diagnostics::~Diagnostics" << std::endl;
                _frequency_status.clear();
                std::cout << "_frequency_status.size() = " << _frequency_status.size() << std::endl;

                _crnt_temp.clear();
                _updater.reset();
                std::cout << "Diagnostics::~Diagnostics-Done" << std::endl;
            }
            ...
            {
                _frequency_status[name] = std::make_shared<diagnostic_updater::HeaderlessTopicDiagnostic>(name, *_updater, param);
            }
            void Tick(const std::string& name)
            {
                _frequency_status[name]->tick();
        ...
        private:
            ...
            std::map<std::string, std::shared_ptr<diagnostic_updater::HeaderlessTopicDiagnostic> > _frequency_status;
